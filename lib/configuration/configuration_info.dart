...
class ConfigInfoPage extends StatelessWidget {
  ///  Class Key
  const ConfigInfoPage({Key? key}) : super(key: key);
  @override
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      children: [
        ...
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 5),
          child: const Text('Esta funcionalidad permite al usuario con '
              'permisos adecuados agregar empresas, sedes y usuarios.'),
        ),
        // Características
        Container(
        ...
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 5),
          child: const Text('Se desplegará un formulario en pantalla que '
              'permitirá al usuario agregar la información correspondiente.\n'),
        ),
        Container(
            ...
            Container(
              ...
              child: ElevatedButton(
                ...
                onPressed: () async {
                  ...
                  if (user.helpScreen) {
                    final request = await user.loadHelpScreen('configuracion');
                    if (request == 'Done') {
                      user.configurationHelp = false;
                    }
                  } else {
                    user.configurationHelp = true;
                  }
