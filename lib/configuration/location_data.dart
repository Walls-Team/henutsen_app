...
class LocationDataPage extends StatelessWidget {
  ///  Class Key
  const LocationDataPage({Key? key}) : super(key: key);
  @override
...
class LocationData extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (location.creationMode) {
      ...
      _companyNames.add(location.currentSearchCompany.name!);
    }
      ...
      child: ListView(
        children: [
          ...
          Padding(
            ...
            child: Text('Detalles de la ubicación',
                        ...
                        : SizedBox(
                            ...
                            child: Container(
                              ...
                              child: DropdownButton<String>(
                                isExpanded: true,
                                value: location.currentSearchCompany.name,
                                ...
                                onChanged: (newValue) {
                                  ...
                                  location
                                      .changeSearchCompany(_newSelectedCompany);
                                },
                                items: _companyNames
                ...
                TableRow(
                  children: [
                    ...
                    Container(
                      ...
                      child: TextFormField(
                        initialValue: location.tempLocation?.name,
                        ...
                        onChanged: (value) {
                          location.tempLocation?.name = value;
                        },
                        maxLength: 30,
                        validator: (value) {
                          if (value!.isEmpty) {
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      if (_formKey.currentState!.validate()) {
                        ...
                        location.tempLocation!.images = [];
                        final chain = jsonEncode(location.tempLocation);
                        var _result = '';
                        var _success = false;
                        var _dialogText = '';
                        //print(chain);
                        ...
                        if (location.creationMode) {
                          ...
                          if (await HenutsenDialogs.confirmationMessage(
                              context,
                              '¿Confirma creación de la ubicación '
                              '${location.tempLocation?.name}?')) {
                            final _cCode =
                                location.currentSearchCompany.companyCode;
                            _result = await location.newLocation(
                            ...
                            if (_result == 'Ok') {
                              _success = true;
                              _dialogText = 'Ubicación registrada exitosamente';
                            } else {
                              _dialogText = 'Error registrando ubicación.\n'
                                  '$_result.\n'
                          ...
                          if (await HenutsenDialogs.confirmationMessage(
                              context,
                              '¿Confirma modificación de la ubicación '
                              '${location.tempLocation?.name}?')) {
                            _result = await location.modifyLocation(
                                imageModel.imageArray,
                                chain,
                                location.currentSearchCompany.companyCode!,
                                user.name2show,
                            ...
                            if (_result == 'Ok') {
                              _success = true;
                              _dialogText = 'Ubicación modificada exitosamente';
                            } else {
                              _dialogText = 'Error modificando ubicación.\n'
                                  '$_result.\n'
                                  'Revise e intente nuevamente.';
                            }
                          }
                                  ...
                                  onPressed: () async {
                                    if (_success) {
                                      ...
                                      if (company.currentCompany ==
                                          location.currentSearchCompany) {
                                        if (!location.creationMode) {
                                          company.currentCompany.locations!
                                              .remove(location.oldName);
                                          company.places
                                              .remove(location.oldName);
                                        }
                                        company.loadLocations(
                                            [location.tempLocation!.name!]);
                                      } else {
                                        if (!location.creationMode) {
                                          location
                                              .currentSearchCompany.locations!
                                              .remove(location.oldName);
                                        }
                                        location.currentSearchCompany.locations!
                                            .add(location.tempLocation!.name!);
                                        location.editDone();
                                      }
                                      if (user.currentUserRole
                                        ...
                                        if (!company.fullCompanyList
                                            .contains(company.currentCompany)) {
                                          company.fullCompanyList
                                              .add(company.currentCompany);
                                        }
                                      }
                                      location.changeSearchCompany(
