...
class TransferModel extends ChangeNotifier {
  /// Lista de traslados detectados en la empresa
  List<AssetTransfer> transfersList = <AssetTransfer>[];

  /// Lista de permisos de la empresa
  List<Authorization> authorizationsList = <Authorization>[];
  ...
  ///estado
  status statusAutho = status.idle;

  /// Reiniciar todas las variables de este modelo
  void resetAll() {
    statusAutho = status.idle;
    transfersList = <AssetTransfer>[];
  ...
  ///cambiar estado
  void asigneStatus(status value) {
    statusAutho = value;
    notifyListeners();
  }
  ...
  Future<String> getTransfers(String companyCode) async {
    ...
    try {
      ...
      final response = await http.get(
          Uri.parse(Config.getTransfersURL + paramString),
      ...
      if (response.statusCode == 200) {
        ...
        if (temp is List) {
          for (final element in temp) {
            transfersList.add(AssetTransfer.fromJson(element));
          }
          return 'Listado recibido';
        } else {
          return 'Error obteniendo autorizaciones (tipo de datos).';
        }
      } else {
        return 'Error obteniendo autorizaciones.';
      }
    } on Exception {
  ...
  Future<String> getAuthorizations(String companyCode) async {
    ...
    try {
      // Armar la solicitud GET/POST con la URL de la página y el parámetro
      final response = await http.get(
          Uri.parse(Config.getAuthorizationsURL + paramString),
          headers: Config.authorizationHeader(Config.userToken));
      ...
      if (response.statusCode == 200) {
        ...
        if (temp is List) {
          ...
          for (final element in temp) {
            authorizationsList.add(Authorization.fromJson(element));
          }
          return 'Listado recibido';
        } else {
          return 'Error obteniendo autorizaciones (tipo de datos).';
  ...
  Future<HttpHenutsenResponse> newAuthorization(String thingToSend) async {
    ...
    try {
      ...
      final response = await http.post(Uri.parse(Config.newAuthorizationURL),
  ...
  Future<HttpHenutsenResponse> modifyAuthorization(String thingToSend) async {
    ...
    try {
      ...
      final response = await http.patch(
          Uri.parse(Config.modifyAuthorizationURL),
          body: thingToSend,
          headers: headers);
      return HttpHenutsenResponse.fromJson(jsonDecode(response.body));
    } on Exception {
  ...
  /// Función para hacer petición POST y crear autorizaciones
  Future<HttpHenutsenResponse> newAuthorizations(String thingToSend) async {
    final headers = Config.authorizationHeader(Config.userToken)
      ..addAll({'Content-Type': 'application/json'});
    try {
      // Armar la solicitud con la URL de la página y el parámetro
      final response = await http.post(Uri.parse(Config.newAuthorizationsURL),
          body: thingToSend, headers: headers);
      return HttpHenutsenResponse.fromJson(jsonDecode(response.body));
    } on Exception {
      return HttpHenutsenResponse(
  ...
  Future<String> deleteAuthorization(String authorizationToDelete) async {
    ...
    try {
      final response = await http.delete(
          Uri.parse(Config.deleteAuthorizationURL),
          body: authorizationToDelete,
          headers: headers);
