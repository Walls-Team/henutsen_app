...
// ----------------------------------------------------
// -------Modelo de Inventario para Provider----------
// ----------------------------------------------------
...
class InventoryModel extends ChangeNotifier {
  ...
  NearAntenna nearAntenna = NearAntenna.notNear;
  ...
  void resetAll() {
    ...
    currentSearchField = '';
    lastReportId = '';
    nearAntenna = NearAntenna.notNear;
  ...
  void updateNearAntenna(NearAntenna newStatus) {
  ...
  /// para decir si estoy para contar activos por salida o normal
  bool out = false;

  ///poner en el fullInventory los activos a verificar en su salida
  void asigneAssetVerify(List<String> assets) {
    final listAux = <Asset>[];
    for (final itemA in assets) {
      for (final itemF in fullInventory) {
        if (itemF.assetCode == itemA) {
          listAux.add(itemF);
        }
      }
    }
    out = true;
    localInventory = listAux;
    notifyListeners();
  }
  ...
  */
  ...
  Future<String> sendReport(
      String thingToSend, String companyCode, String op) async {
  ...
  /// Función para hacer petición POST y crear el activo
  Future<String> newAsset(List<PlatformFile> files2send, String thingToSend,
      int quantity, String user, String userName) async {
    // Armar la solicitud con la URL de la página y el parámetro
    // Armar petición multiparte
    final url = Uri.parse(Config.newAssetURL);
    ...
    try {
      ...
      if (streamedResponse.statusCode == 200) {
        ...
        try {
          ...
          if (response.body == 'Nuevos activos agregados') {
            return 'Ok';
          } else {
  ...
  Future<String> modifyAsset(String thingToSend, String user,
      {List<PlatformFile> files2send = const <PlatformFile>[]}) async {
    // Armar petición multiparte
  ...
  /// Función para hacer petición PATCH y modificar varios activos
  Future<String> modifySeveralAssets(String listToSend, String user) async {
    final customHeaders = Config.authorizationHeader(Config.userToken)
      ..addAll({'Content-Type': 'application/json'});
    try {
      // Armar petición
      final response = await http.patch(
          Uri.parse(Config.modifySeveralAssetsURL),
          body: listToSend,
          headers: customHeaders);

      if (response.statusCode == 200) {
        if (response.body == 'Activos modificados') {
          return 'Ok';
        } else {
          return response.body;
        }
  ...
  /// Función para hacer petición DELETE y eliminar el activo
  Future<String> deleteAsset(String assetData) async {
    final customHeaders = Config.authorizationHeader(Config.userToken)
      ..addAll({'Content-Type': 'application/json'});
    ...
    try {
      ...
      final response = await http.delete(
        Uri.parse(Config.deleteAssetURL),
        body: assetData,
        headers: customHeaders,
      );
      ...
      if (responseBody is Map<String, dynamic>) {
        if (responseBody['statusCode'] as int == 200) {
          if ((responseBody['message'] as String)
              .startsWith('Se eliminó el activo')) {
            return 'Ok';
          } else {
            return response.body;
          }
